...
const Database = function({ db: { host, port, keyspace} } = {}) {
  let dbContacts = [];
  if (Array.isArray(host)) dbContacts = host;
  else dbContacts.push(host);

  const dbOptions = {
    clientOptions: {
      contactPoints: dbContacts,
  ...
  const checkSchema = function checkSchema () {
  ...
  const openConnection = function openConnection () {
  ...
  const sendError = function sendError (err) {
